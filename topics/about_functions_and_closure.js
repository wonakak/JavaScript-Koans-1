...
test("using call to invoke function",function(){
    ...
    /*another way to invoke a function is to use the call function
       which allows you to set the callers "this" context.
        Call(args..) can take any number of arguments:
            1) the first one is always the context that ['this' should be set to] in the called function,
            2) and the arguments to be sent to the function,
...
test("using apply to invoke function",function(){
    ...
    Apply only has two arguments:
        1) the first is the context that this should be set to in the called function
        2) and the second is the array of arguments to be passed into the called function.*/
